...
class StructuredRecord:
    ...
    def compute_hash(self) -> str:
        """Hash canonical content — volatile instance IDs excluded — so identical
        evidence yields an identical verdict fingerprint (replayable), while any
        change to values/results/resolutions still changes the hash."""
        VOLATILE = {"fact_id", "assertion_id", "resolution_id"}
        ...
        data = {
            "facts": [strip_ids(f) for f in self.facts],
            "assertions": [strip_ids(a) for a in self.assertions],
            "resolutions": [strip_ids(r) for r in self.resolutions],
