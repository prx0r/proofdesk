...
class FoxitPDFClient:
    ...
    async def merge(self, doc_ids: list[str]) -> dict:
        ...
        Foxit requires 2+ documents. If only 1, upload it again for merge.
        """
        if not self.is_configured:
...
async def run_signing_pipeline(
    ...
    if gate_result.allowed == GateResult.ALLOW:
        log_event("STAGE_START", "system", {"stage": "esign"})
        try:
            # Try Foxit first, then fallbacks
            from .esign_fallback import esign_document
            esign_result = await esign_document(
                document_bytes, f"{case_id}.pdf", signer_email
            )
            result.foxit_esign = {
                "provider": esign_result.provider,
                "status": esign_result.status,
                "folder_id": esign_result.folder_id,
                "signing_url": esign_result.signing_url,
            }
            log_event("ESIGN", "system", {
                "provider": esign_result.provider,
                "operation": "create_folder",
                "folder_id": esign_result.folder_id,
                "signer": signer_email,
                "irreversible": True,
                "detail": esign_result.detail,
            })
